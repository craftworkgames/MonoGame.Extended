...
/// </remarks>
[ContentImporter(".ldtk", ".world", ".tilemapworld",
    DefaultProcessor = "TilemapWorldProcessor",
    DisplayName = "Tilemap World Importer - MonoGame.Extended")]
public sealed class TilemapWorldImporter : ContentImporter<TilemapProjectContentItem>
{
    ...
    private static readonly XmlSerializer s_tiledSerializer = new XmlSerializer(
        typeof(TiledMapXml),
        new[]
        {
    ...
    private static TilemapData LoadAndConvertTiledMap(
        string mapFilePath,
    ...
    private static void ResolveTiledTilesetPaths(
        TiledMapXml map,
        string baseDirectory,
        string mapFilePath,
        ContentImporterContext context)
            ...
            if (string.IsNullOrWhiteSpace(tilesetRef.Source))
            {
                if (tilesetRef.Image != null && !string.IsNullOrWhiteSpace(tilesetRef.Image.Source))
                {
                    string absoluteImagePath = Path.GetFullPath(
                        Path.Combine(baseDirectory, tilesetRef.Image.Source));
    ...
    private static void RegisterTiledLayerDependencies(
        List<TiledLayerXml> layers,
        string baseDirectory,
        ContentImporterContext context)
    {
    ...
    private static TilemapProjectContentItem ImportTilemapWorldTiled(
        string worldFilePath,
        TilemapWorldDefinition definition,
    ...
    private static TilemapProjectContentItem ImportTilemapWorldOgmo(
        string worldFilePath,
        TilemapWorldDefinition definition,
        string worldDirectory,
        ContentImporterContext context)
    ...
    private static void RegisterOgmoTilesetDependencies(
        OgmoProject project,
