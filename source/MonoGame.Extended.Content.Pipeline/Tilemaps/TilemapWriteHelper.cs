...
internal static class TilemapWriteHelper
{
    ...
    private static void WriteImageLayer(ContentWriter writer, TilemapImageLayerData imageLayer, IExternalReferenceRepository refs)
    {
        ExternalReference<Texture2DContent> textureRef =
            refs.GetExternalReference<Texture2DContent>(imageLayer.TexturePath ?? string.Empty);
    ...
    {
        if (obj is TilemapTileObjectData)     { return TilemapObjectTypeByte.Tile; }
        if (obj is TilemapEllipseObjectData)  { return TilemapObjectTypeByte.Ellipse; }
        if (obj is TilemapPointObjectData)    { return TilemapObjectTypeByte.Point; }
        if (obj is TilemapPolygonObjectData)  { return TilemapObjectTypeByte.Polygon; }
        if (obj is TilemapPolylineObjectData) { return TilemapObjectTypeByte.Polyline; }
        if (obj is TilemapTextObjectData)     { return TilemapObjectTypeByte.Text; }
        return TilemapObjectTypeByte.Rectangle;
    }
}
