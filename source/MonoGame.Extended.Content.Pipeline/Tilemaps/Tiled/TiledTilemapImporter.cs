...
public sealed class TiledTilemapImporter : ContentImporter<TilemapProjectContentItem>
{
    ...
    private static void ResolveTilesetPaths(
        TiledMapXml map,
    ...
    private static void RegisterLayerDependencies(
    ...
    private static void RegisterImageLayerDependency(
        TiledImageLayerXml imageLayer,
        string sourceDirectory,
        ContentImporterContext context)
    {
        ...
        string absoluteImagePath = Path.GetFullPath(
            Path.Combine(sourceDirectory, imageLayer.Image.Source));
        ContentLogger.Log($"Adding dependency '{absoluteImagePath}'");
        context.AddDependency(absoluteImagePath);
