...
namespace MonoGame.Extended.Content.Pipeline.Tiled
{
    ...
    /// <summary>
    /// Processes Tiled map files into runtime content.
    /// </summary>
    ...
    {
        /// <summary>
        /// Processes the Tiled map content.
        /// </summary>
        /// <param name="contentItem">The imported map content item.</param>
        /// <param name="context">The processor context.</param>
        /// <returns>The processed map content item with built external references.</returns>
        public override TiledMapContentItem Process(TiledMapContentItem contentItem, ContentProcessorContext context)
        {
            try
            {
                ...
                // Check for unsupported orientations
                if (map.Orientation == TiledMapOrientationContent.Hexagonal ||
                    map.Orientation == TiledMapOrientationContent.Staggered)
                    throw new NotSupportedException($"{map.Orientation} Tiled Maps are currently not implemented!");

                ContentLogger.Log($"Processing Tiled map '{map.FilePath}'");

                ProcessTilesets(contentItem, map.Tilesets, context);
        ...
        private static void ProcessTilesets(TiledMapContentItem contentItem, IReadOnlyList<TiledMapTilesetContent> tilesets, ContentProcessorContext context)
        {
            foreach (var tileset in tilesets)
            {
                if (string.IsNullOrWhiteSpace(tileset.Source))
                {
                    // embedded tileset, build the texture reference
                    if (tileset.Image != null)
                    {
                        ContentLogger.Log($"Building texture reference for embedded tileset: {tileset.Image.Source}");
                        TiledContentBuildHelpers.BuildTextureReference(contentItem, context, tileset.Image);
                    }
                }
                else
                {
                    // External tileset, build the tileset reference
                    ContentLogger.Log($"Building tileset reference: {tileset.Source}");
                    TiledContentBuildHelpers.BuildTilesetReference(contentItem, context, tileset.Source);
                }
            }
        }
            ...
            foreach (var layer in layers)
            {
                switch (layer)
                {
                    case TiledMapImageLayerContent imageLayer:
                        ...
                        TiledContentBuildHelpers.BuildTextureReference(contentItem, context, imageLayer.Image);
