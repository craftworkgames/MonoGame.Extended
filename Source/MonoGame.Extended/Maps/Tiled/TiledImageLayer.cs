...
namespace MonoGame.Extended.Maps.Tiled
{
    public class TiledImageLayer : TiledLayer, IMovable
    {
        ...
        public TiledImageLayer(string name, Texture2D texture, Vector2 position, int depth)
            : base(name, depth)
        {
            Position = position;
            _texture = texture;
            ...
            _imageVertices[0] = new VertexPositionTexture(new Vector3(position, depth), new Vector2(0f, 0f));
            _imageVertices[1] = new VertexPositionTexture(new Vector3(position.X + texture.Width, position.Y, depth), new Vector2(1f, 0f));
            _imageVertices[2] = new VertexPositionTexture(new Vector3(position.X, position.Y + texture.Height, depth), new Vector2(0f, 1f));
            _imageVertices[3] = new VertexPositionTexture(new Vector3(position.X + texture.Width, position.Y + texture.Height, depth), new Vector2(1f, 1f));
