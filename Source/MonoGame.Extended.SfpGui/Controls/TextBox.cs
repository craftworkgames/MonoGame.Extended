...
namespace MonoGame.Extended.SfpGui.Controls {
    public sealed class TextBox : Control {
        ...
        public override Size GetContentSize (IGuiContext context) {
            ...
            var stringSize = font.MeasureString (Text ?? string.Empty).ToSize ();
            return new Size ((int) stringSize.Width,
                (int) (stringSize.Height < font.Size ? font.Size : stringSize.Height));
        }

        //protected override Size2 CalculateDesiredSize(IGuiContext context, Size2 availableSize)
        //{
        //    var font = Font ?? context.DefaultFont;
        //    return new Size2(Width + Padding.Left + Padding.Right, (Height <= 0.0f ? font.Size + 2 : Height) + Padding.Top + Padding.Bottom);
        //}
        ...
        private int FindNearestGlyphIndex (IGuiContext context, Point position) {
            ...
            foreach (var glyph in font.GetGlyphRects (textInfo.Position, textInfo.Text)) {
                var fontRegionWidth = glyph.Width;// ?? 0;
                var glyphMiddle = (int) (glyph.X + fontRegionWidth * 0.5f);
        ...
        //protected override string CreateBoxText(string text, DynamicSpriteFont font, float width)
        //{
        //    return text;
        //}

        private Rectangle GetCaretRectangle (TextInfo textInfo, int index) {
            var caretRectangle = textInfo.Font.GetTextBounds (textInfo.Position, textInfo.Text.Substring (0, index));
